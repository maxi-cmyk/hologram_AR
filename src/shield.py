...
class EnergyShield:
    ...
    def draw(self, frame, anchor, scale):
        ...
        # Draw glowing core
        cv2.circle(frame, (cx, cy), int(base_radius * 0.3), self.core_color, -1)
        cv2.circle(frame, (cx, cy), int(base_radius * 0.35), self.primary_color, 2)
        cv2.circle(frame, (cx, cy), int(base_radius * 0.4), self.primary_color, 1)

        # Draw Inner Hexagon
        self._draw_polygon(frame, cx, cy, int(base_radius * 0.7), 6, rot1, self.primary_color, 3, draw_spokes=True)
        
        # Draw Middle Octagon
        self._draw_polygon(frame, cx, cy, int(base_radius * 1.0), 8, rot2, self.secondary_color, 2, draw_spokes=False)
        
        # Draw Outer Dodecagon (12 sided)
        self._draw_polygon(frame, cx, cy, int(base_radius * 1.3), 12, rot3, self.primary_color, 1, draw_spokes=False)
        
        # Draw floating particles or energy rings
        for i in range(4):
            ring_radius = int(base_radius * 1.3) + int(math.sin(current_time * 5 + i) * 10 * scale)
            cv2.circle(frame, (cx, cy), ring_radius, self.secondary_color, 1)
